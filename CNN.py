...
class CNN(Module):
    def __init__(self):
        ...
        self.relu = nn.ReLU()
        self.fc = MLP([64 * 12 * 12, 256, 10], batch_norm = False)
        
    def execute(self, x):
        '''
        x: jittor array of shape (batch, 3, 28, 28)
        '''
        batch = x.shape[0]
        x = self.relu(self.conv1(x))
        x = self.relu(self.bn(self.conv2(x)))
        x = self.max_pool(x)
        x = jt.reshape(x, (batch, -1))
        x = self.fc(x)
        return x
...
plt.xlabel('Epoch')
plt.ylabel('Loss(CEL)')
plt.title('CNN result (modified) Learning rate: %.2f' % lr)
...
print('Avg Test Accuracy: %.4f %%' % acc)
